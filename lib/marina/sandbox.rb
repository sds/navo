...
module Marina
  ...
  class Sandbox
    ...
    def install_cookbooks
      ...
      vendored_cookbooks_dir = File.join(directory_path, 'cookbooks')
      ...
      @suite.copy(from: File.join(directory_path, 'cookbooks', '.'),
                  to: File.join(@suite.chef_run_dir, 'cookbooks'))
    end

    def install_chef_directories
      puts 'Creating/copying directories...'
      %w[data_bags environments roles].each do |dir|
        @suite.copy(from: File.join(@suite.repo_root, dir),
                    to: File.join(@suite.chef_run_dir, dir))
      end
    end
    ...
    def directory_path
      @directory_path ||=
        File.join(@suite.repo_root, '.marina', 'suites', @suite.name).tap do |path|
